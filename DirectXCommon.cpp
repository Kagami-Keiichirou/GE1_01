...
void DirectXCommon::FixFPSInitialize()
{
	//現在時刻を記録
	reference_ = std::chrono::steady_clock::now();
}

void DirectXCommon::FixFPSUpdate()
{
	const std::chrono::microseconds kMinTime(uint64_t(1000000.0f / 60.0f));
	const std::chrono::microseconds kMinTimes(uint64_t(1000000.0f / 65.0f));
	...
	//1/60秒経っていない場合
	if (elapsed < kMinTimes) {
		//1/60経過するまで、最小なループを繰り返す
